...
> 离线自检：`265 项全部通过`（Linux + .NET 6.0.301，`tools/MCLCS.SelfCheck`）
...
| 标记 | 含义 |
|---|---|
| ✅ | 核心逻辑已实现，且经 `MCLCS.SelfCheck` 离线验证（265/265） |
...
| # | 功能 | 状态 | 说明 |
|---|---|---|---|
| 49 | 第一层：总开关 | 🖥️ | 「启用 AI 助手」默认关闭；关闭时下方全部隐藏、零资源占用 |
| 50 | 第二层：部署方式单选 | 🖥️ | 外部 API（默认选中，零下载）/ 本地部署（Ollama，约 1-3 GB） |
| 51 | 本地部署：Ollama 检测与安装 | 🔧 | `OllamaManager.DetectAsync`（解析版本号）；`InstallAsync` 下载安装器+进度+取消+清理临时；已安装显示绿灯+版本 |
| 52 | 本地部署：模型拉取 | 🔧 | `OllamaManager.PullModelAsync`（进度+取消）；目录 3 款：Qwen2.5-Coder-1.5B(0.9G)/InternLM2-1.8B(1.1G)/Phi-3.5-mini-3.8B(2.2G)；未下载显示「下载模型 (X.XGB)」，完成后「已就绪」 |
| 53 | 本地部署：切换模型确认窗 | 🖥️ | 切到未下载模型弹轻量确认（InternLM 1.1GB / Phi 2.2GB），取消则回退 |
| 54 | 本地部署：服务状态灯 | 🖥️ | `OllamaServiceStatus` → 绿(运行)/黄(启动中)/红(未启动)，`OllamaStatusToBrushConverter` |
| 55 | 外部 API 配置 | 🖥️ | API 地址 / Key（密文 PasswordBox）/ 模型；`TestConnectionCommand` 测试连接 |
| 56 | 外部 API 模型自动补全 | ✅ | `Assistant.SuggestModelForEndpoint`：openai→gpt-4o-mini、deepseek→deepseek-chat、127.0.0.1/localhost→空、其它→空 |
| 57 | 第三层：功能子开关 | 🖥️ | 崩溃解读 / 推荐理由生成 / Mod 描述翻译 三项独立开关 |
| 58 | 本地部署推理路由 | ✅ | `Assistant` 在本地模式调用 Ollama `/api/chat`（失败回退本地启发式） |
| 59 | 外部 API 推理路由 | ✅ | `Assistant` 在外部模式调用 OpenAI 兼容接口（失败回退本地启发式） |
| 60 | Assistant.Config 持久化 | 🔧 | `AiConfig`（Enabled/Mode/SelectedLocalModel/Endpoint/Key/Model/三项开关）随 `LauncherProfile` 保存 |
...
| # | 功能 | 状态 | 说明 |
|---|---|---|---|
| 70 | SelfCheck 265 项 | ✅ | `tools/MCLCS.SelfCheck` 覆盖全部核心模块 + v1.1 新增 Toolbox/AI/Instance/Playtime/Updater/Ollama |
...
| 文件 | 大小 | SHA256 |
|---|---|---|
| `MCLCS-v1.1-source.zip` | 249 KB | `8c7f99175189c7811aff6bf1e287626a53dca2f4be9b78383d166f07d2f9f0f7` |
| `MCLCS-v1.1-full.tar.gz` | 316 KB | `314ad3c0b43c9d0bdf51d5752f9e041f350b36204063facc14b45c1ad4dd1050` |
| `MCLCS-v1.1-download.html` | 776 KB | 内嵌 base64 双按钮下载页（含上述 SHA256） |
